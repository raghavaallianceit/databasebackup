CREATE OR REPLACE PROCEDURE cx_cvciq_v3.test_visit_overview_report (
        ...
        (
            ...
            FROM
                (
                    ...
                    FROM
                        bi_request r,
                        bi_request_activity_day b,
                        bi_request_catering_activity c,
                        bi_request_act_day_room rm 
                    WHERE
                        r.state = 'CONFIRMED'
                        ...
                        AND c.request_activity_day_id = b.id
                        AND r.id = rm.request_id
                      --  AND c.room=rm.room 
                        AND rm.room_type='MAIN_ROOM'
                        --AND c.request_activity_day_id in (select request_activity_day_id from bi_request_act_day_room where room_type ='MAIN_ROOM') 
