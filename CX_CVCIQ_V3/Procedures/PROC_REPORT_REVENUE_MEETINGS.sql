CREATE OR REPLACE PROCEDURE cx_cvciq_v3.proc_report_revenue_meetings (
    out_chr_err_code   OUT                VARCHAR2,
    out_chr_err_msg    OUT                VARCHAR2,
    out_meeting_tab    OUT                return_meeting_revenue_arr,
    in_type            IN                 VARCHAR2,
    in_year            IN                 NUMBER,
    in_month           IN                 NUMBER,
    ...
    lrec                 return_meeting_revenue_report;
...
BEGIN
    out_meeting_tab := return_meeting_revenue_arr();
...
--                         || l_month);
    LOOP
        ...
        lrec := return_meeting_revenue_report();
        out_meeting_tab := return_meeting_revenue_arr(return_meeting_revenue_report());
...
--						 dbms_output.put_line('Inside cursor   '  );
            BEGIN
                ...
                lrec := return_meeting_revenue_report();
                lrec.value := l_cur_meeting_data(i).opportunityrevenue;
                lrec.name := l_cur_meeting_data(i).value;
                IF l_num_counter > 1 THEN
                    out_meeting_tab.extend();
                    out_meeting_tab(l_num_counter) := return_meeting_revenue_report();
                ELSE
                    out_meeting_tab := return_meeting_revenue_arr(return_meeting_revenue_report());
                END IF;
